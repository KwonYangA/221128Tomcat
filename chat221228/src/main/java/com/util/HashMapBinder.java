...
//Spring 부트에서는 RequestParam 대신 해줌
//Model, ModelMap
//사용자가 입력한 값을 Map에 담아준다
//담을 Map은 컨트롤계층에서 bind메소드 호출시 파라미터를 이용해서
//원본 주소번지를 받아온다
//그리고 그 안에 담는다
public class HashMapBinder {
 ...
 //생성자 파라미터에 요청객체가 필요한 이유는 뭐죠?
 public HashMapBinder(HttpServletRequest req) {
	 //생성자의 1역할 - 전변의 초기화 
	 this.req = req;
	 realFolder = "C:\\Users\\user1\\Desktop\\tomcat\\chat221228\\src\\main\\webapp\\pds";
 }
 public void multiBind(Map<String,Object> pMap) {
	 logger.info("multiBind 호출");
	 //컨트롤러 계층에서 생성한 맵 객체 비우기
	 pMap.clear();
	 try {
		multi = new MultipartRequest(req, realFolder, maxSize, encType, new DefaultFileRenamePolicy());
	} catch (Exception e) {
		logger.info(e.toString());//발생할 예외 이름 출력하기
	}
	 ...
	 if(files!=null) {
		 //업로드 대상 파일을 객체로 만듦
		 File file = null; //내용까지 복제되는 건 아니다 - 파일명에 대해서만 객체화
