...
public class MemberController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	 ...
	 //오라클 서버에서 연동하는 일만 전담하는 XXXDao클래스 인스턴스화
	 MemberDao mDao = new MemberDao();
	 //사용자가 화면에 입력한 아이디 요청하기
	 String mem_id = req.getParameter("mem_id");
	 //사용자가 화면에 입력한 비번 요청하기
	 String mem_pw = req.getParameter("mem_pw");
	 //mabatis에 파라미터로 아이디와 비번을 Map에 담아서 넘겨야 하니까 Map생성
	 ...
	 //위에서 받아온 rmap에 들어있는 그러니까오라클 서버에서 select문으로 조회한 결과를
	 //꺼내서 세션 객체에 저장함
	 ...
	 //이 때는 세션을 가지고 있으니 세션값을 꺼내서 XXX님 환영하빈다. 라고 말할 수 있음 - 인증 구현 끝
